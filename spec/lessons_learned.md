...
## §7. 角色標記功能（Speaker Attribution，2026-02-09）

### 7.1 從「靜音模式」改為「角色標記模式」

| 項目 | 內容 |
|------|------|
| **日期** | 2026-02-09 |
| **問題** | 原始「我在說話」功能使用靜音模式（停止收錄），但用戶希望能區分對話雙方 |
| **原設計** | Spacebar Toggle → 靜音/取消靜音 |
| **新設計** | Spacebar HOLD → 角色切換（按住=我，放開=對方） |

**核心變更**：

| 面向 | 現有行為 | 新行為 |
|------|---------|--------|
| Spacebar | Toggle（按一下開/關） | HOLD（按住=我，放開=對方） |
| Web Speech | mute/unmute（停止累積） | 持續運行，只改標記 |
| Segment | 無 speaker 欄位 | `speaker: 'them' \| 'me'` |
| 視覺 | 全螢幕覆蓋層 | 底部小指示器 + 段落標記 |
...
```javascript
function pttStart() {
    // 1. 先輸出當前 buffer（標記為上一個說話者 'them'）
    if (smartSegmenter.buffer.trim()) {
...
| 說話者 | 邊框顏色 | 背景 Tint | 標籤 |
|--------|---------|-----------|------|
| 對方 (them) | 綠色 | `rgba(0, 255, 136, 0.08)` | 👤 對方 |
| 我 (me) | 藍色 | `rgba(0, 212, 255, 0.08)` | 🙋 我 |
...
**問題**：匯出的 Markdown 沒有包含說話者標記

**解決方案**：更新 `exportTranscript()` 函數
1. 使用 `span[style*="font-weight: 600"]` 選擇器找到說話者標籤
2. Markdown 標題格式：`### 👤 對方 (12:34:56)` 或 `### 🙋 我 (12:34:56)`

**關鍵教訓**：
- 修改 UI 渲染後，匯出邏輯也要同步更新
- 使用 CSS 選擇器時，注意 inline style 的精確匹配
